...
const STAMPS_TABLE = process.env.DYNAMODB_TABLE_STAMPS || 'CareerPassportStamps';
const NFTS_TABLE = process.env.DYNAMODB_TABLE_NFTS || 'CareerPassportNFTs';
...
export async function createStamp(stamp) {
  const params = {
    TableName: STAMPS_TABLE,
    Item: {
      stampId: stamp.stampId || `stamp_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
      userId: stamp.userId,
...
export async function createNFT(nft) {
  const params = {
    TableName: NFTS_TABLE,
    Item: {
      nftId: nft.nftId || `nft_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
      userId: nft.userId,
      tokenId: nft.tokenId,
      name: nft.name,
