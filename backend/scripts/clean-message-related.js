/**
 * メッセージ関連データベースをクリーンにするスクリプト
 *
 * 用途: メッセージ、イベント応募、マッチングテーブル内のデータを削除（テーブル自体は削除しません）
 * 対象テーブル:
 *   - NonFungibleCareerMessages: メッセージデータ
 *   - NonFungibleCareerEventApplications: イベント応募データ（応募者情報）
 *   - NonFungibleCareerMatches: マッチングデータ（学生と企業のマッチング情報）
 *
 * 注意: このスクリプトはテーブル内のデータのみを削除します。
 *       テーブル構造やインデックスは保持されます。
 *
 * 実行方法: node scripts/clean-message-related.js
 */
 ...
 * メッセージ関連テーブルをクリーンアップ
 */
async function cleanMessageRelated() {
  console.log("=== メッセージ関連データベースクリーンアップ開始 ===\n");
  console.log(
...
rl.question(
  "⚠️  警告: メッセージ、イベント応募、マッチングテーブル内のすべてのデータを削除します（テーブル自体は削除しません）。続行しますか？ (yes/no): ",
  (answer) => {
