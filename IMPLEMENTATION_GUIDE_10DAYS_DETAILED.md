...
すべての画面をモックデータで完成させ、UI の動作を確認する。WalletConnect認証とVC設計を実装する。
...
#### ステップ 2: WalletConnect統合（2-3 時間）

**作業**:

1. **WalletConnectコンポーネント** (`frontend/src/components/WalletConnect.jsx`)
   - MetaMask接続機能の実装
   - ウォレットアドレス表示
   - 接続状態の管理
...
2. **認証フローの変更**
   - 個人ログイン/登録ページを削除
   - 企業ログイン/登録ページを削除
   - WalletConnectによる認証に統一

3. **UserTypeSelectionページの更新**
   - WalletConnectによる直接認証に変更
...
1. `frontend/src/router.jsx` を確認・更新
   ...
   **企業ユーザー向けルート**:
   ...
   - マッチング人材: `/org/matched-candidates`
   - メッセージ: `/org/messages`
   - 企業VC管理: `/org/settings`

2. **Layoutコンポーネントの統合**
   - StudentLayout、OrgLayoutでルートをラップ
...
#### ステップ 4: 共通コンポーネントの作成（2-3 時間）

**作業**:

1. **Layout コンポーネント**
   - `StudentLayout.jsx`: 個人ユーザー向けレイアウト
     - ナビゲーションバー（全ページへのリンク）
     - WalletConnectコンポーネント統合
   - `OrgLayout.jsx`: 企業ユーザー向けレイアウト
     - ナビゲーションバー（全ページへのリンク）
     - WalletConnectコンポーネント統合
...
#### ステップ 5: VC設計への移行（3-4 時間）

**作業**:

1. **個人VC管理ページ** (`frontend/src/pages/VCAndZKP.jsx`)
   - VC（Verifiable Credential）管理タブ
   - ゼロ知識証明タブ
   - VCの追加・削除機能（モック）
   - ZKP生成・検証機能（モック）

2. **企業VC管理ページ** (`frontend/src/pages/OrgSettings.jsx`)
   - 企業向けVC管理
   - 法人登記証明書、税務証明書などのVC（モック）
   - VCの追加・削除機能

3. **個人情報設定ページの削除**
   - `/student/settings`をVC管理ページに変更
...
#### ステップ 11: イベント・求人・メッセージ機能の実装（4-5 時間）

**作業**:

1. **個人ユーザー向けページ**
   - イベント一覧 (`StudentEvents.jsx`)
   - イベント応募 (`StudentEventApply.jsx`)
   - 求人条件設定 (`StudentJobConditions.jsx`)
   - 求人探し (`StudentJobSearch.jsx`)
   - マッチング企業 (`StudentMatchedCompanies.jsx`)
   - メッセージ (`StudentMessages.jsx`) - 個人情報非表示設計

2. **企業ユーザー向けページ**
   - イベント一覧 (`OrgEvents.jsx`)
   - イベント共同オファー (`OrgEventCollaborate.jsx`)
   - 人材募集条件設定 (`OrgRecruitmentConditions.jsx`)
   - 人材探し (`OrgCandidateSearch.jsx`)
   - マッチング人材 (`OrgMatchedCandidates.jsx`)
   - メッセージ (`OrgMessages.jsx`) - 個人情報非表示設計

3. **メッセージUI改善**
   - ウォレットアドレス表示（個人名は表示しない）
   - VCからの選択的情報開示機能
   - NFT証明書情報の表示（公開情報）
...
- ✅ すべての画面が完成（個人11ページ、企業11ページ）
- ✅ WalletConnect統合完了（MetaMask認証）
- ✅ VC設計への移行完了
- ✅ メッセージUI改善完了（個人情報非表示設計）
- ✅ ナビゲーションバー実装完了
- ✅ ローカルストレージでデータ管理
- ✅ モックデータで動作確認可能
- ✅ エラーハンドリング実装済み
...
## Day 5-6: バックエンドAPI実装 ⏳ **予定**

### 目標

イベント、メッセージ、マッチング機能のバックエンドAPIを実装する。

### 作業内容

#### ステップ 1: データベース設計（2-3 時間）

**作業**:

1. **DynamoDBテーブル設計**
   - イベント応募テーブル（ウォレットアドレスベース）
   - メッセージテーブル（ウォレットアドレスベース）
   - マッチングテーブル（ウォレットアドレスベース）

2. **テーブル作成スクリプト**
   - `backend/scripts/create-tables.js`の更新
...
#### ステップ 3: フロントエンドとバックエンドの統合（2-3 時間）
...
## Day 7-8: ウォレット連携と統合・機能完成 ⏳ **予定**

### 目標

MetaMask と連携し、UI からブロックチェーンを操作できるようにする。すべての機能を完成させる。
...
**最終更新**: 2025年1月（WalletConnect設計・VC設計実装完了時点）
